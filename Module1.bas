...
'server xhr get 字符串 (GetDataStr 已经弃用)
    ...
    Else
        If CStr(XMLHTTP.Status) = "404" Then
            MsgBox "HTTP错误 404 连接失败" & vbCrLf & vbCrLf & "此错误可能是当地的运营商导致，" & vbCrLf & "在模拟器助手设置中更换下载源可能会解决。", vbCritical
        ElseIf CStr(XMLHTTP.Status) = "504" Then
            MsgBox "HTTP错误 504 服务器处理超时" & vbCrLf & vbCrLf & "请等五秒钟之后重开模拟器助手再试一次。", vbCritical
        Else
...
'获取 Yuzu Early Access 版本号
    ...
    If AlwaysUseCloudFlare = "True" Or AlwaysUseCloudFlare = "False" Then
    ...
    Else ' GitHub Never
        Set objJson = JSON.parse(GetDataStr2("https://api.github.com/repos/pineappleea/pineapple-src/releases"))
    End If
...
'获取 Yuzu 主线版版本号
    ...
    Else ' GitHub Never
    Set objJson = JSON.parse(GetDataStr2("https://api.github.com/repos/yuzu-emu/yuzu-mainline/releases/latest"))
    End If
...
'获取 Ryujinx 版本号
    ...
    Else ' GitHub Never
        Set objJson = JSON.parse(GetDataStr2("https://api.github.com/repos/Ryujinx/release-channel-master/releases/latest"))
    End If
...
'获取 Ryujinx CN 版本号
    ...
    Else ' GitHub Never
        TmpML = GetDataStr2("https://api.github.com/repos/YidaozhanYa/RyujinxCN/releases/latest")
    End If
...
'检查更新
    ...
    If AlwaysUseCloudFlare = "Never" Then
        Set objJson = JSON.parse(GetDataStr2("https://api.github.com/repos/YidaozhanYa/NSEmuHelper/releases/latest"))
    Else
